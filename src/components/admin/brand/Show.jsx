...
const Show = () => {
  ...
  const deleteBrand = async (brandId) => {
    if (window.confirm("Are you sure you want to delete this category?")) {
      try {
        const res = await deleteBrandService(brandId);
        if (res.status === 200) {
                ...
                <Table responsive hover className="mb-0">
                  ...
                  <tbody>
                    {loading ? (
                      ...
                      brands.map((brand) => (
                        <tr key={brand.id}>
                          <td className="ps-4">{brand.id}</td>
                          <td className="fw-semibold">{brand.name}</td>
                          <td><span className="text-sm fw-normal">{brand.slug}</span></td>
                          <td>
                            {brand.status === 1 ? (
                      ...
                      <tr>
                        <td colSpan="5" className="text-center py-4 text-muted">
                          <NoState message="No brands found." />
                        </td>
                      </tr>
                    )}
                  </tbody>
