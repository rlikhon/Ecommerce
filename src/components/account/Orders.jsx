...
import { getCustomerOrdersService } from "../../services/OrderServices.ts";
...
const Orders = () => {
  ...
  const fetchOrderHistory = useCallback(async () => {
    ...
    try {
      ...
      console.error(
        "Failed to sync customer order history registry lines:",
        error,
      );
    } finally {
      setLoading(false);
  ...
  const getStatusBadgeConfig = (status) => {
    switch (status?.toLowerCase()) {
      case "delivered":
        return { bg: "success", text: "Delivered" };
      case "pending":
        return { bg: "warning", text: "Pending" };
      case "processing":
        return { bg: "warning", text: "Processing" };
      case "shipped":
        return { bg: "info", text: "In-Transit" };
      case "cancelled":
        return { bg: "danger", text: "Cancelled" };
      case "failed":
        return { bg: "danger", text: "Failed" };
      default:
  ...
  const getPaymentStatusBadgeConfig = (payment_status) => {
    switch (payment_status?.toLowerCase()) {
      case "paid":
        return { bg: "success", text: "Paid" };
      case "unpaid":
        return { bg: "warning", text: "Unpaid" };
      case "partial_paid_due":
        return { bg: "warning", text: "Partial Payment - Due" };
      case "refunded":
        return { bg: "danger", text: "Refunded" };
      case "failed":
        return { bg: "danger", text: "Failed" };
      default:
        return { bg: "secondary", text: payment_status };
    }
  };
            ...
            <Card className="customer-orders-card border-0 shadow-sm mb-4">
              <Card.Header className="bg-white border-0 pt-4 pb-2 px-4">
                <h5 className="fw-bold mb-0 text-dark d-flex align-items-center gap-2">
                  <ShoppingBag size={18} className="text-secondary" /> Complete
                  Purchase History
                </h5>
                <p className="text-muted small mb-0 mt-1">
                  Track tracking coordinates and fulfillment records for past
                  purchases.
                </p>
              </Card.Header>
                ...
                {loading ? (
                  ...
                  <div className="text-center py-5 text-muted">
                    <p className="fs-6 mb-3">
                      You haven't placed any platform store purchases yet.
                    </p>
                    <Link
                      to="/shop"
                    ...
                    <Table className="align-middle mb-0">
                      <thead className="table-light">
                        <tr>
                          ...
                          <th>
                            <Calendar size={13} className="me-1" /> Order Date
                          </th>
                          <th>
                            <DollarSign size={13} className="me-1" /> Grand
                            Total
                          </th>
                          <th>
                            <Activity size={13} className="me-1" /> Status
                          </th>
                          <th>
                            <Activity size={13} className="me-1" /> Payment
                            Status
                          </th>
                      ...
                      <tbody>
                        {orders.map((order) => {
                          ...
                          const badgeConfigPayment =
                            getPaymentStatusBadgeConfig(order.payment_status);
                          return (
                            <tr key={order.id || order.order_number}>
                              ...
                              <td>
                                <Badge
                                  bg={badgeConfig.bg}
                                  className="badge-fulfillment shadow-none"
                                >
                                  {badgeConfig.text}
                                </Badge>
                              </td>
